...
List::List(const List &copyList)     
{
	...
	ListNode * pCur = copyList.mpHead, * pPrev = nullptr;

	while (pCur) {
		
		// equivalent to (ListNode *)malloc(sizeof(ListNode));
...
List & List::operator= (const List &rhs) // overloaded assignment operator - must perform a deep copy
{
	...
	// first need to free the list before assignment
	this->destroyListHelper(this->mpHead);

	ListNode * pCur = rhs.mpHead, *pPrev = nullptr;

	while (pCur) {

		// equivalent to (ListNode *)malloc(sizeof(ListNode));
		ListNode * pMem = new ListNode;
		pMem->setData(pCur->mData);
		pMem->setNextPtr(nullptr);
