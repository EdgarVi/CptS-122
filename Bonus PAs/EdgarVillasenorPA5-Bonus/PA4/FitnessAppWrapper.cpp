...
void FitnessAppWrapper::performUserSelection(const int &option)
{
	...
	case 4: // store weekly EP
		...
		cout << "Excercise plans stored to excercisePlans.txt!" << endl;
		break;
...
void FitnessAppWrapper::storeWeeklyExcercisePlan()
{
	...
	{
		this->mFstrEP.open("excercisePlans.txt"); // opens in input/output mode by default with fstream object
...
{
	fileStream >> plan; // extract 1 ExcercisePlan from the file through the fileStream -
						// uses overloaded file stream extraction operator -
